...
class PlaygroundArgumentsSpec extends FlatSpec with MustMatchers with GeneratorDrivenPropertyChecks {
  "Arguments#parseQueryString" must "create configuration" in {
    ...
    PlaygroundArguments().parseQueryString("?sfen=lnsgkgsnl%2F1r5b1%2Fppppppppp%2F9%2F9%2F9%2FPPPPPPPPP%2F1B5R1%2FLNSGKGSNL%20b%20-%200%207g7f&mlang=en") mustBe PlaygroundArguments(
      sfen = Some("lnsgkgsnl/1r5b1/ppppppppp/9/9/9/PPPPPPPPP/1B5R1/LNSGKGSNL b - 0 7g7f"),
      config = PlaygroundConfiguration(messageLang = English)
    )
  }
}
...
class PlaygroundArgumentsBuilderSpec extends FlatSpec with MustMatchers with GeneratorDrivenPropertyChecks  {
  "ArgumentsBuilder#toRecordUrl" must "include comments" in StateCache.withCache { implicit cache =>
    ...
    PlaygroundArgumentsBuilder(GameControl(g.copy(newComments = comments))).toRecordUrl mustBe expected
  }
