...
import com.mogproject.mogami.frontend.model.analyze.{AnalyzeCompleted, AnalyzeStarted, CheckmateAnalyzeResult}
...
case class AnalyzeCheckmateAction(timeoutSec: Int, started: Boolean) extends PlaygroundAction {
  override def execute(model: BasePlaygroundModel): Option[BasePlaygroundModel] = {
    if (!started) {
      Some(model.copy(newAnalyzeResult = Some(CheckmateAnalyzeResult(AnalyzeStarted, timeoutSec, None))))
    } else {
      model.mode.getGameControl.map { gc =>
        ...
        model.copy(newAnalyzeResult = Some(CheckmateAnalyzeResult(AnalyzeCompleted, timeoutSec, result)))
      }
    }
  }
}
