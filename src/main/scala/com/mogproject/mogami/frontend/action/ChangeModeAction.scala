  ...
  override def execute(model: BasePlaygroundModel): Option[BasePlaygroundModel] = if (model.mode.modeType == newModeType) {
    ...
    (model.mode, model.mode.getGameControl, newModeType) match {
      ...
      case (mode, Some(gc), EditModeType) if gc.isInitialState || confirmed =>
        Some(model.copy(
          newMode = EditMode(gc.game.gameInfo, gc.getDisplayingTurn, gc.getDisplayingBoard, gc.getDisplayingHand, Some(mode.isHandicapped)))
        )
      ...
      case (EditMode(gi, t, b, h, _), None, _) =>
        Try(GameControl(Game(Branch(State(t, b, h), isFreeMode = model.config.freeMode), gameInfo = gi))) match {
          case Success(gc) =>
            Some(model.copy(newMode = createMode(newModeType, gc, None)))
          case Failure(e) =>
      ...
      // Play -> View | View -> Play
      case (mode, Some(gc), _) =>
        Some(model.copy(newMode = createMode(newModeType, gc, Some(mode.isHandicapped))))
