...
trait CircularEffectorLike extends ForegroundEffectorLike[Square] {
  ...
  private[this] def generateAnimateElems: Seq[TypedTag[SVGElement]] =
    Seq("r" -> finalRadius, "opacity" -> opacityValues).map { case (n, v) => createAnimateElem(n, v, duration) }

  override def generateElements(x: Square): Seq[TypedTag[SVGElement]] =
    Seq(svgBoard.getRect(x).center.toSVGCircle(initialRadius, cls := "board-move board-selecting", generateAnimateElems))
}
